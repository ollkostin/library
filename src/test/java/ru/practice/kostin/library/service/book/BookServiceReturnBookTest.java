...
@SpringBootTest
public class BookServiceReturnBookTest {
    ...
    @Test
    public void whenReturnBookExpectSuccess() throws Exception {
        book = new Book(isn, name, author, user);
        when(bookDao.get(isn)).thenReturn(book);
        when(userDao.getById(1)).thenReturn(user);
        when(bookDao.update(book)).thenReturn(1);
        assertEquals(1, bookService.returnBook(bookDto.getIsn(), user.getId()));
    }

    @Test(expected = NotFoundException.class)
    public void whenReturnBookShouldThrowNotFoundExceptionWithBookReason() throws Exception {
        when(bookDao.get(isn)).thenReturn(null);
        bookService.returnBook(bookDto.getIsn(), user.getId());
    }

    @Test(expected = NotFoundException.class)
    public void whenReturnBookShouldThrowNotFoundExceptionWithUserReason() throws Exception {
        book = new Book(isn, name, author, user);
        when(bookDao.get(isn)).thenReturn(book);
    ...
    public void whenReturnBookShouldThrowNotAcceptableExceptionWithUserMismatchReason() throws Exception {
    ...
    public void whenReturnBookShouldThrowAffectedRowsCountMismatchException() throws Exception {
        book = new Book(isn, name, author, user);
        when(bookDao.get(isn)).thenReturn(book);
        when(userDao.getById(any())).thenReturn(user);
        when(bookDao.update(book)).thenReturn(0);
        assertNotEquals(1, bookService.returnBook(bookDto.getIsn(), user.getId()));
    }
}
